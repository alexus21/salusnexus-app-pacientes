              ...
              >
                ...
                {{ subscriptionStatus }}
                    ...
                    <div class="subscription-details">
                        ...
                        <div v-if="isLoadingPlan" class="plan-features-loading">Cargando características...</div>
                        <div v-else-if="planDetails" class="modern-plan-card"
                             :class="{'plan-premium': isPremium, 'plan-free': !isPremium}">
                            <div class="plan-card-header">
                <span class="plan-badge-modern" :class="{'premium-badge': isPremium, 'free-badge': !isPremium}">
                  {{ isPremium ? (planDetails.recommended ? 'RECOMENDADO' : 'ADVANCED') : 'FREE' }}
                </span>
                            </div>
                            <div class="plan-card-title">{{ planDetails.name }}</div>
                            <div class="plan-card-price">
                <span class="plan-price-value">
                  {{ isPremium ? `$${parseFloat(planDetails.price_monthly).toFixed(2)}` : '$0' }}
                </span>
                                <span class="plan-price-desc">/ Por miembro</span>
                            </div>
                            <ul class="plan-card-features">
                                <li v-for="feature in planDetails.features" :key="feature.id">
                                    <i class="fas fa-check-circle feature-icon-modern"></i> {{ feature.feature }}
                                </li>
                            </ul>
                            <button
                                v-if="!isPremium && user.subscription_type === 'paciente_gratis'"
                                class="plan-action-main-btn premium-btn"
                                @click="navigateToSubscriptionPlans"
                            >
                                Cambiar a Premium
                            </button>
                            ...
                            <div class="plan-card-note">
                                {{
                                    isPremium ? 'Se requiere tarjeta de débito/crédito' : 'No se requiere tarjeta de débito/crédito'
                                }}
                            </div>
                        </div>
                        <div v-else class="plan-features-empty">No hay características registradas para este plan.</div>
...
export default {
    ...
    watch: {
        'user.subscription_type': {
            handler(newType) {
                if (newType) this.fetchPlanDetails(newType);
            },
            immediate: true
        }
    },
    ...
    methods: {
        ...
        async fetchPlanDetails(type) {
            this.isLoadingPlan = true;
            try {
                const API_URL = process.env.VUE_APP_API_URL;
                const response = await fetch(`${API_URL}/subscription-plan/${type}`);
                const data = await response.json();
                if (response.ok && data) {
...
.modern-plan-card {
    background: #fff;
    border-radius: 18px;
    box-shadow: 0 6px 32px rgba(13, 110, 253, 0.10), 0 1.5px 3px rgba(0, 0, 0, 0.03);
...
.plan-action-main-btn:hover {
    background: #0d6efd;
}

.plan-action-main-btn.premium-btn {
    background: #0d6efd;
    color: #fff;
    cursor: default;
}
